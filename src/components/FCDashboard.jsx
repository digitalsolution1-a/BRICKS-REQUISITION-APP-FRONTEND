      ...
      <main className="max-w-7xl mx-auto p-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4 mt-4">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <input 
              type="text" 
              placeholder="SEARCH BY STAFF OR REF..." 
            ...
            />
            <button onClick={exportToExcel} className="bg-black text-white px-8 py-3 rounded-2xl text-[10px] font-black hover:bg-[#A67C52] transition-all shadow-lg">
              EXPORT CSV
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {activeTab === 'queue' ? (
            filterList(requisitions).map(req => (
              <div key={req._id} className="bg-white rounded-[2.5rem] border border-gray-100 p-6 flex flex-col md:flex-row justify-between items-center gap-6 shadow-sm hover:shadow-md transition-all">
                ...
                <button onClick={() => setSelectedReq(req)} className="w-full md:w-auto bg-black text-white px-10 py-4 rounded-2xl text-[10px] font-black tracking-[0.2em] shadow-xl hover:bg-[#A67C52] transition-all">
                  VET REQUEST
                </button>
              </div>
            ))
          ...
          {filterList(activeTab === 'queue' ? requisitions : history).length === 0 && (
            <div className="text-center py-32 bg-white rounded-[3rem] border-4 border-dashed border-gray-50">
              <p className="text-gray-300 font-black tracking-[0.4em] text-xs uppercase underline decoration-[#A67C52]">No records found</p>
            </div>
          )}
        </div>
      </main>
