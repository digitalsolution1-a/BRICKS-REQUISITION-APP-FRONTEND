...
function EditRequisition() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (submissionData.currency === 'Others' && submissionData.otherCurrency) {
      submissionData.currency = submissionData.otherCurrency;
    }
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-2xl rounded-[3rem] overflow-hidden border-t-[12px] border-[#A67C52]">
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {formData.currency === "Others" && <div className="flex flex-col"><label className="text-[9px] text-gray-500 mb-1">Specify Currency</label><input name="otherCurrency" value={formData.otherCurrency} onChange={handleInputChange} className="bg-gray-50 p-4 rounded-xl border-b-2" /></div>}
