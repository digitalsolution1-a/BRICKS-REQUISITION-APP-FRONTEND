...
const MDDashboard = () => {
  ...
  const getHODComment = (historyArray) => {
    ...
    return hodEntry ? { comment: hodEntry.comment, name: hodEntry.actorName, priority: hodEntry.priority || 'Normal' } : null;
  };
  ...
  const computeMDApprovedTotals = () => {
    const approvedList = filterList(history).filter((req) => {
      ...
      if (isCurrentlyRejected) return false;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <section className="space-y-4">
              ...
              {filterList(inbox).map(req => {
                ...
                const hodInfo = getHODComment(req.approvalHistory);
                const isUrgent = hodInfo && String(hodInfo.priority || '').toUpperCase() === 'URGENT';

                return (
                  <div key={req._id} className={`bg-white p-6 rounded-[2.5rem] border shadow-sm flex justify-between items-center hover:shadow-md transition-all animate-in slide-in-from-left-2 ${isUrgent ? 'border-amber-400 ring-2 ring-amber-200' : 'border-gray-100'}`}>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-[8px] font-black text-gray-400">{req.department}</p>
                        {isUrgent && (
                          <span className="inline-flex items-center gap-1 bg-amber-100 text-amber-700 text-[8px] font-black px-2 py-0.5 rounded-full animate-pulse">
            ...
            <section className="space-y-4">
              ...
              {filterList(backlog).map(req => {
                ...
                return (
                  <div key={req._id} className={`bg-white p-6 rounded-[2.5rem] border-l-4 border-l-red-500 shadow-sm flex justify-between items-center opacity-80 hover:opacity-100 transition-all ${isUrgent ? 'ring-2 ring-amber-200' : 'border-gray-100'}`}>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        {isUrgent && (
                          <span className="inline-flex items-center gap-1 bg-amber-100 text-amber-700 text-[8px] font-black px-2 py-0.5 rounded-full animate-pulse">
        ...
        {activeTab === 'all' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filterList(allDepartments).map(req => {
              ...
              return (
                <div key={req._id} className={`bg-white p-6 rounded-[2.5rem] border shadow-sm flex justify-between items-center hover:shadow-md transition-all animate-in slide-in-from-top-2 ${isUrgent ? 'border-amber-400 ring-2 ring-amber-200' : 'border-gray-100'}`}>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
          ...
          <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-8 md:p-12 overflow-y-auto max-h-[90vh]">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    {(() => {
                      ...
                      if (hod && String(hod.priority || '').toUpperCase() === 'URGENT') {
                        return (
                          <span className="inline-flex items-center gap-1.5 bg-amber-100 border border-amber-300 text-amber-800 text-[9px] font-black px-3 py-1 rounded-full animate-pulse shadow-sm">
              ...
              <div className="space-y-6 mb-10">
                ...
                {/* HOD AND FC REMARKS PLACED WITH AMBER FLASH LIGHT FOR URGENT PRIORITY BEFORE FC REMARKS */}
                <div className="space-y-4">
                  {(() => {
                    const hod = getHODComment(selectedReq.approvalHistory);
                    const fc = getFCComment(selectedReq.approvalHistory);
                    if (!hod && !fc) return null;
                    const isUrgent = hod && String(hod.priority || '').toUpperCase() === 'URGENT';

                    return (
                      <>
                        {hod && (
                          <div className={`p-6 rounded-r-[2rem] border transition-all ${
                            isUrgent 
                              ? 'bg-amber-50/80 border-l-8 border-l-amber-500 border-amber-200 shadow-md ring-2 ring-amber-300' 
                              : 'bg-blue-50 border-l-4 border-blue-500 border-blue-100'
                          }`}>
                            <div className="flex items-center justify-between mb-2">
                              <p className={`text-[9px] font-black uppercase tracking-[0.2em] ${isUrgent ? 'text-amber-700' : 'text-blue-500'}`}>
                                Head of Dept Remarks ({hod.name}) {isUrgent && '— [URGENT PRIORITY SET]'}
                              </p>
                              {isUrgent && (
                                <div className="flex items-center gap-1.5 bg-amber-200 text-amber-900 px-3 py-1 rounded-full text-[8px] font-black animate-pulse shadow-inner">
                                  <span className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-ping"></span>
                                  FLASHING AMBER ACTIVE
                                </div>
                              )}
                            </div>
