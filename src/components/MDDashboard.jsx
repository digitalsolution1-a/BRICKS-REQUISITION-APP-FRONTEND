...
const MDDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetching Pending Queues and History concurrently
      const [mdRes, fcRes, historyRes] = await Promise.all([
        ...
        axios.get(`${API_BASE_URL}/requisitions/history/MD`, { 
          headers: { Authorization: `Bearer ${token}` } 
        }).catch(() => ({ data: [] })) // Fallback if endpoint is empty
      ]);
  ...
  const handleAction = async (id, action, isOverride = false) => {
    ...
    try {
      ...
      setSelectedReq(null);
      setMdComment('');
      fetchData(); // Refreshes both pending and history streams
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#FBFBFB] uppercase">
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        {activeTab === 'pending' ? (
          ...
          /* HISTORY TABLE - FILTERED BY HISTORY STATE */
