...
const FCDashboard = () => {
  ...
  // Helper to render priority styling badge
  const renderPriorityBadge = (priority) => {
    const p = String(priority || 'NORMAL').toUpperCase();
    if (p === 'URGENT' || p === 'HIGH') {
      return <span className="bg-red-50 text-red-600 text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-tighter border border-red-100">🔥 {p}</span>;
    }
    if (p === 'MEDIUM') {
      return <span className="bg-amber-50 text-amber-600 text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-tighter border border-amber-100">⚡ {p}</span>;
    }
    return <span className="bg-gray-100 text-gray-500 text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-tighter">📋 {p}</span>;
  };
  ...
  const exportToExcel = () => {
    ...
    const headers = "ID,Date,Requester,Department,Priority,Vendor,Amount,Currency,Status\n";
    const data = dataToExport.map(r => {
      const cleanedAmount = parseFloat(String(r.amount || '0').replace(/[^0-9.]/g, '')) || 0;
      return `${r._id},${new Date(r.createdAt).toLocaleDateString()},${r.requesterName},${r.department},${r.priority || 'Normal'},${r.vendorName || 'N/A'},${cleanedAmount},${getStandardCurrency(r.currency)},${r.status}`;
    }).join("\n");
