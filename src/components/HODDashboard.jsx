...
const HODDashboard = () => {
  ...
  // Find selected request from any of the lists
  const selectedReq = 
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/requisitions/action/${id}`, {
        action, actorRole: 'HOD', actorName: user.name, comment: hodComment || (action === 'Approved' ? 'Departmental approval granted.' : '')
      ...
      <main className="max-w-7xl mx-auto p-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4 mt-4">
          <div>
            ...
            <div className="flex gap-6 mt-6">
              {['queue', 'history', 'all'].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`text-[10px] font-black tracking-widest pb-2 border-b-2 transition-all ${activeTab === tab ? 'border-[#A67C52] text-black' : 'border-transparent text-gray-400 hover:text-black'}`}>
                  {tab === 'queue' ? `PENDING (${requisitions.length})` : tab === 'history' ? `HISTORY (${history.length})` : `ALL DEPT (${allDepartments.length})`}
                </button>
              ))}
            </div>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <button onClick={exportData} className="bg-black text-white px-8 py-3 rounded-2xl text-[10px] font-black hover:bg-[#A67C52]">EXPORT CSV</button>
          </div>
        </div>

        {/* Display Content based on Tabs */}
          ...
          {activeTab === 'queue' ? (
            filterList(requisitions).map(req => (
              <div key={req._id} className="bg-white rounded-[2.5rem] border border-gray-100 p-6 flex justify-between items-center shadow-sm">
                <div>
                    <h3 className="font-black text-gray-900">{req.requesterName}</h3>
                    <p className="text-[9px] text-gray-400">{req.vendorName}</p>
                </div>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100">
               <h3 className="text-xs font-black text-gray-900 mb-4">DEPARTMENTAL OVERVIEW</h3>
               {filterList(allDepartments).map(req => (
                   <div key={req._id} className="p-4 border-b border-gray-50 flex justify-between text-[10px]">
                       <span>{req.requesterName}</span>
                       <span className="font-black text-[#A67C52]">{req.status}</span>
                   </div>
               ))}
            </div>
          )}
          ...
          <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl p-12 overflow-y-auto max-h-[90vh]">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {[
                ...
                { label: "Amount", value: `${selectedReq.currency} ${selectedReq.amount?.toLocaleString()}` },
                ...
                <div key={idx} className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  <p className="text-[8px] font-black text-gray-400 uppercase truncate tracking-widest">{item.label}</p>
                  <p className="text-[10px] font-black text-gray-800 truncate">{item.value}</p>
                </div>
              ))}
            </div>
            ...
            {activeTab === 'queue' && (
              <div className="mt-8 pt-8 border-t">
                <textarea value={hodComment} onChange={(e) => setHodComment(e.target.value)} placeholder="Remarks..." className="w-full p-6 bg-gray-50 rounded-2xl mb-4 text-xs font-bold" />
                <div className="flex gap-4">
                  <button onClick={() => handleAction(selectedReq._id, 'Approved')} className="flex-1 bg-[#A67C52] text-white py-4 rounded-2xl text-[10px] font-black">APPROVE</button>
                  <button onClick={() => handleAction(selectedReq._id, 'Declined')} className="flex-1 bg-white border-2 border-red-500 text-red-500 py-4 rounded-2xl text-[10px] font-black">DECLINE</button>
                </div>
              </div>
            )}
            <button onClick={() => setSelectedReqId(null)} className="mt-6 w-full text-center text-[9px] font-black text-gray-400">CLOSE MODAL</button>
          </div>
        </div>
      )}
    </div>
